// 然后给我介绍了5分钟的cache，问我知道有什么cache的策略，最后就来了一个LRU，
// 有点小改动，给了一个时间区间，只有（当前时间 - 时间区间）内的结果才有效，也不麻烦，需要多判断一个if，
// 就是如果get时值存在但是却不在时间区间内的话，需要更新linkedhashmap， 再返回null。再就是，
// 返回结果的时候如果是null，要返回Optional type。
